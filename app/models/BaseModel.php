...
//     {
//         $this->db = new Database(); // Replace with your actual database connection setup
//     }
...
//     {
//         $stmt = $this->db->getConnection()->prepare("SELECT COUNT(id) AS total_records FROM {$this->tableName} WHERE 1=1" . $this->where);
...
//         // Count total records
//         $countQuery = "SELECT COUNT(*) AS total_records FROM {$this->tableName} WHERE 1=1" . $this->where;
...
//         $query = "SELECT * FROM {$this->tableName} WHERE 1=1" . $this->where . $this->orderBy . " LIMIT {$offset}, {$itemsPerPage}";
//         $stmt = $this->db->getConnection()->prepare($query);
//         foreach ($this->params as $key => $value) {
//             $paramType = is_int($value) ? PDO::PARAM_INT : (is_bool($value) ? PDO::PARAM_BOOL : (is_null($value) ? PDO::PARAM_NULL : PDO::PARAM_STR));
//             $stmt->bindValue($key + 1, $value, $paramType); // Bind by position
