...
class Migration
{
    ...
    {
        $blueprint = new TableBlueprint($tableName); // Pass tableName to TableBlueprint constructor
        $callback($blueprint);
...
//             $refTableName = $key['refTableName'];
//             $refColumnName = $key['refColumnName'];
//             $newConstraintName = "fk_" . $columnName . "_" . $tableName;

//             // Check if the foreign key constraint already exists
//             $constraintExists = $this->checkConstraintExists($tableName, $columnName);

//             // If the constraint exists, modify its name before dropping
//             if ($constraintExists) {
//                 $newConstraintName = $newConstraintName . "_renamed". $tableName;
//                 $dropSql = "ALTER TABLE $tableName DROP FOREIGN KEY fk_$columnName";
//                 $this->db->getConnection()->exec($dropSql);
//             }

//             // Add the foreign key constraint
//             $constraintSql = "ALTER TABLE $tableName ADD CONSTRAINT $newConstraintName FOREIGN KEY ($columnName) REFERENCES $refTableName($refColumnName)";
//             $this->db->getConnection()->exec($constraintSql);
//         }
//     }
...
//     public function generateForeignKeySQL($tableName, $columnName, $refTableName, $refColumnName)
//     {
//         $sql = "ALTER TABLE $tableName ADD CONSTRAINT fk_$columnName FOREIGN KEY ($columnName) REFERENCES $refTableName($refColumnName)";
//         return $sql;
//     }

//     // Function to check if the foreign key constraint exists
//     function checkConstraintExists($tableName, $columnName)
//     {
//         $sql = "SELECT COUNT(*) AS count FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = '$tableName' AND CONSTRAINT_NAME = 'fk_$columnName' AND CONSTRAINT_TYPE = 'FOREIGN KEY'";
//         $result = $this->db->getConnection()->query($sql);
//         $row = $result->fetch();
//         return $row['count'] > 0;
//     }
